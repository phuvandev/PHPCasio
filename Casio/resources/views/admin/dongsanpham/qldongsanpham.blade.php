            ...
            <div class="x_content">
              ...
              <div style=" margin-bottom: 10px;">
                <a class="text-muted font-13 m-b-30">
                  <form action="/qldongsanpham_create" method="GET">
              ...
              <table id="datatable" class="table table-striped table-bordered table-hover">
                <thead>
                    <tr>
                      <th style="text-align: center;">Mã DSP</th>
                ...
                <tbody>
                  @foreach ($dongsanpham as $d)
                    <tr>
                      <td style="text-align: center;">{{$d->id}}</td>
                      <td style="text-align: center;">{{$d->TenDSP}}</td>
                      <td style="text-align: center;">
                        @if($d->AnhDaiDien != '')
                          <img  style="width: 140px; height: 100px;" src="assets/custom/Anh/DSP/{{$d->AnhDaiDien}}"/>
                        @else
                          <img alt="Không có ảnh"/>
                        @endif
                      </td>
                      <td>{{Str::limit($d->MoTa, 200) }}</td>
                      <td style="text-align: center;">{{$d->TenMenu}}</td>
                      <td style="text-align: center;">
                        <form action="/qldongsanpham_update/{{$d->id}}" method="GET">
                      ...
                      <td style="text-align: center;">
                        <form action="/deleteDongSanPham/{{$d->id}}" method="POST" id="delete-form">
                          @csrf
                          @method('DELETE')
              ...
              <div class="paginate" style="text-align: center">
                {{$dongsanpham->appends(request()->all())->links()}}
              </div>
              <!-- End phân trang-->
            </div>
          </div>
