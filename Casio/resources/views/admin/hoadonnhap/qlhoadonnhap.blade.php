          ...
          <div class="x_panel">
            ...
            <div class="x_content">
                ...
                <div style=" margin-bottom: 10px;">
                    ...
                    <form action="/qlhoadonnhap_create" method="GET">
                ...
                <table id="datatable" class="table table-striped table-bordered table-hover">
                    <thead>
                        <tr>
                            <th style="text-align: center;">Mã HĐN</th>
                            <th style="text-align: center;">Tên HĐN</th>
                    ...
                    <tbody>
                    @foreach ($hoadonnhap as $h)
                        <tr>
                            <td style="text-align: center;">{{$h->id}}</td>
                            <td style="text-align: center;">{{$h->TenHDN}}</td>
                            <td style="text-align: center;">{{$h->name}}</td>
                            <td style="text-align: center;">{{$h->TenNCC}}</td>
                            <td style="text-align: center;">
                              {{date_format(date_create($h->created_at), "l, d/m/Y H:i:s")}}
                            </td>

                            <td style="text-align: center;"> 
                              <form action="/qlhoadonnhap_detail/{{$h->id}}" method="GET">
                            ...
                            <td style="text-align: center;">
                                <form action="/qlhoadonnhap_update/{{$h->id}}" method="GET">
                            ...
                            <td style="text-align: center;">
                                <form action="/deleteHoaDonNhap/{{$h->id}}" method="POST" id="delete-form">
                                @csrf
                                @method('DELETE')
                ...
                <div class="paginate" style="text-align: center">
                    {{$hoadonnhap->appends(request()->all())->links()}}
                </div>
                <!-- End phân trang-->
            </div>
          </div>
